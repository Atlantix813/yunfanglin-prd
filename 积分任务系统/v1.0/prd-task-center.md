...
| 积分数据 | PC端 | 居民积分排行、商家核销排行、统计卡片 | 已设计 |
...
- 用户完成一次行为，系统创建一条任务明细
- 自动发放任务：积分直接到账，周期进度 +1
- 手动领取任务：创建[待领取奖励]明细，用户无需先领取即可继续完成下一次，每次完成生成新的待领取明细
...
| 17 | 完善个人信息 | 完善个人信息 | 将个人信息所有必填项全部完善即可获得积分 | 5 | 单次 | 仅 1次 | 用户在个人信息页成功提交完善信息的节点触发 | 【个人信息】编辑页 | 旧用户上线迁移时补建待领取记录 |
...
- **排序交互**：可排序列的列头带排序图标，点击切换排序维度（始终降序，不支持升序切换），当前排序列高亮；默认按[累计获取]降序
- **分页**：默认每页20条
