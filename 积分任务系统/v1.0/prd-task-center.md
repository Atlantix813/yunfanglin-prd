...
| 我的账户 | C端 | 余额展示、过期提示、赚积分入口、隐藏支付积分、积分桶规则 | 已设计 |
| 订阅消息 | C端 | 子开关管理、任务完成提醒、积分过期提醒 | 已设计 |
| 积分兑换 | C端 | 去赚积分入口、积分不足弹窗改造、非本社区兑换拦截 | 已设计 |
...
2. 如果不存在：创建新桶，桶月份=当月，过期时间=创建月份+12个月（如2027年5月1日00:00:00），桶状态=正常
3. 如果已存在：直接累加到该桶
4. 写入对应的积分流水记录
5. 更新用户积分余额
