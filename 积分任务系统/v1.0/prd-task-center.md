...
| 赚积分 | C端 | 邀请奖励发放节点调整、邀请卡片统计调整 | 已设计 |
...
- 下一周期开始时，先将未领取的待领取奖励自动发放，再重置为「去完成」（由流程3每日定时任务处理）
...
- **统计口径**：
  - 已邀请人数：[被邀请码]等于当前用户邀请码的用户数
  - 已完善人数：上述用户中已完善个人信息的数量
  - 已赚取积分：邀请奖励类型的积分流水合计
...
| [领取方式] | 手动（用户点击领取）/ 自动（任务配置开启自动发放，或周期重置时系统兜底发放），待领取显示"-" |
...
| 流程4-1 | 积分转账流程（C2C，过期时间继承） | 用户间积分转移时 |
| 流程5 | 积分桶过期清零 | 每月1日定时 |
