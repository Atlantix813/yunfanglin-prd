...
> **版本**：v1.0 &nbsp;|&nbsp; **负责人**：俊杰 &nbsp;|&nbsp; **更新**：2026-04-30 &nbsp;|&nbsp; **状态**：方案探索中
...
| Tab | 排序方式 |
|---|---|
| 按编号 | 按作品编号升序（管理员在后台设定的编号） |
| 按票数 | 按当前票数降序，票数相同按编号升序 |
...
- **筛选区**：支持按[活动名称]（模糊搜索）、[活动状态]（下拉：全部/草稿/报名中/投票中/公示中/已结束/已归档）、[活动分类]（下拉）、[创建时间]（日期范围）组合筛选
- **操作栏**：「创建活动」蓝色按钮
- **表格列**：
...
|---|---|---|
| [启用奖励] | 开关 | 默认关闭 |
| [奖励规则] | 动态表格 | 启用后展开，可添加多行：名次范围（如"第1名"/"第2-3名"/"第4-10名"）+ 奖励积分值 |
...
4. **手机号校验**：若活动开启了[是否需要手机号]，检查用户是否已绑定手机号
5. **单作品重复校验**：
...
7. **投票间隔校验**：距离上一次投票是否超过 3 秒（防自动化脚本）
8. **IP 频率校验**：同一 IP 每分钟投票次数是否超过 30 次

校验 5-8 使用 Redis 原子操作（Lua 脚本），保证高并发下的数据一致性：
...
| 表名 | 用途 | 关键字段 |
|---|---|---|
| 投票活动 | 活动主表，管理活动生命周期 | 活动名称、分类、状态、报名/投票时间、投票规则（JSON）、奖励配置（JSON）、所属社区、公示天数 |
| 活动分组 | 活动下的分组配置 | 活动ID、分组名称、排序序号 |
| 候选作品 | 参赛作品/候选项 | 活动ID、作品名称、介绍、图片列表（JSON）、视频地址、分组ID、报名者用户ID、审核状态、编号、票数（冗余）、提交时间 |
| 投票记录 | 每次投票的流水 | 活动ID、作品ID、投票用户ID、openid、投票IP、投票日期、状态 |
| 奖励记录 | 活动结束后的奖励发放 | 活动ID、作品ID、用户ID、名次、奖励积分值、发放人、发放时间 |
