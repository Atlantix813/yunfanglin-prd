...
| 分类 | 模块 | 端 | 功能点 |
|---|---|---|---|
| C端展示与交互 | 投票活动列表页 | C端 | · [页面结构](#h-1-页面结构)：分类筛选Tab + 活动卡片（封面图+社区标签+状态标签）<br>· [活动卡片结构](#h-2-活动卡片结构)：社区/范围标签、参与数据、操作按钮<br>· [入口](#h-3-入口)：首页金刚区 + 信息流推荐 + 个人中心 |
| C端展示与交互 | 活动详情与投票页 | C端 | · [页面结构](#h-1-页面结构-1)：封面轮播 + 统计面板 + 举办方 + 简介 + 规则色块 + 瀑布流作品<br>· [候选作品卡片](#h-2-候选作品卡片)：4:3/3:4错开瀑布流，底部极简（票数+投票按钮）<br>· [投票交互流程](#h-3-投票交互流程)：成功/已投/用完/资格不符等Toast<br>· [排序切换](#h-4-排序切换)：分段控件（按编号/按票数/社区排名）<br>· [排行榜展示](#h-5-排行榜展示)：总排名 + 社区贡献榜 |
| C端展示与交互 | 作品详情页 | C端 | · [图片/视频轮播](#h-1-页面结构-2)：Swiper + 全屏预览<br>· [作品信息](#h-1-页面结构-2)：编号 + 票数 + 社区内排名 + 参赛者社区 |
| C端展示与交互 | 作品报名提交页 | C端 | · [表单字段](#h-1-表单字段)：作品信息 + 图片/视频上传 + 所属社区自动带入<br>· [内容安全审核](#h-2-内容安全审核)：微信 imgSecCheck / msgSecCheck |
| C端展示与交互 | 我的投票页 | C端 | · [我的投票Tab](#h-2-我的投票-tab)：参与过的活动列表<br>· [我的参赛Tab](#h-3-我的参赛-tab)：审核状态 + 驳回修改 |
| 通知触达 | 订阅消息 | C端 | · [新增子开关](#h-1-新增子开关)：投票活动（3项通知）<br>· [提醒规则](#h-2-提醒规则)：审核结果 + 活动状态变更 + 奖励到账 |
| 后台配置 | 投票活动管理 | PC端 | · [列表页](#h-1-列表页)：筛选 + 发起社区 + 活动范围 + 状态流转操作<br>· [创建/编辑弹窗](#h-2-创建-编辑活动弹窗)：基本信息 + 活动范围 + 时间 + 投票规则 + 分组 + 奖励 |
| 后台配置 | 作品审核管理 | PC端 | · [列表页](#h-1-列表页-1)：所属社区 + 审核状态 + 批量通过 + 驳回弹窗 |
| 数据与奖励 | 投票数据与奖励 | PC端 | · [数据概览面板](#h-2-数据概览面板)：4项统计卡片 + 社区参与统计表<br>· [排名列表](#h-3-排名列表)：所属社区 + 社区内排名 + 冻结/发放操作 |
| 权限 | 权限控制 | PC端 | · [菜单权限](#h-菜单权限)：社区管理员 + 超管分级<br>· [数据权限](#h-数据权限)：按社区隔离 |
| 底表与流程 | 核心业务流程 | - | · [活动状态自动流转](#h-流程1)：定时任务<br>· [投票与防刷校验](#h-流程4详述-投票防刷校验链)：9步校验链 + Redis原子操作 |
| 底表与流程 | 数据模型 | - | · [表结构总览](#h-表结构总览)：5张表<br>· [字段值列表](#h-字段值列表)：状态/范围/类型枚举 |
...
- **封面轮播区**：
  - 全宽，高度 200px，支持多图轮播（自动 4 秒切换 + 手势滑动）
  - 底部圆点指示器
  - 状态栏和导航栏叠加在封面图上（白色文字）
- **白色信息面板**（紧贴封面底部，无间距）：
  - **统计面板**：蓝色渐变底卡片，三项数据横排（参与人数 / 累计票数 / 浏览量），白色数字+标签
  - **活动标题行**：标题（18px加粗）+ 右侧[投票中]状态标签
  - **举办方信息**：灰色小字「XX社区 · 社区居委会 主办」
  - **活动简介**：直接全文展示，不做展开/收起
  - **投票规则色块**：浅灰色圆角色块，左侧规则文字，右上角嵌入倒计时（橙色）
  - **参与范围提示**：蓝色小字「仅限 XX 居民参与」，位于规则色块下方
    - 跨社区活动：「XX街道 辖区居民均可参与」
    - 全平台活动：「全区居民均可参与」
    - 用户不在投票范围内时：「本活动仅限 XX 居民投票，您可浏览但无法投票」
- **候选作品区**（通栏标题，吸顶）：
  - 左侧：「候选作品」标题 + 「共 N 个」计数
  - 右侧：分段排序控件（按编号 / 按票数），选中项白底凸起，未选中灰底
  - 跨社区/全平台活动时增加第三项「社区排名」
- **候选作品列表**：双列瀑布流布局

### 2. 候选作品卡片

双列瀑布流布局（小红书风格），图片高度在 4:3 和 3:4 之间随机错开，形成错落感。视觉重心放在作品图片上，底部信息极简化：

- **作品图片**：首图展示，宽度撑满半列，高度随比例变化
- **编号标签**：左上角贴边标签（深色半透明底白字），如"01号"
- **社区标签**：跨社区/全平台活动时，图片右下角显示作品所属社区名称；本社区活动不显示
- **作品名称**：13px加粗，不限行数（允许自然换行）
- **底部信息行**：票数（橙色数字）+ 投票按钮，同行两端对齐
- **投票按钮**（微圆角矩形，非胶囊）：
  - 可投票：蓝色渐变「投票」
  - 已投票：灰色「已投票」
  - 不在投票范围：灰色「仅限XX居民」，不可点击
  - 活动非投票中：不显示按钮

### 3. 投票交互流程

用户点击作品卡片上的「投票」按钮，系统执行防刷校验链（详见【核心业务流程】），根据结果弹出对应 Toast：

| 场景 | Toast 文案 | 按钮变化 |
|------|-----------|---------|
| 投票成功，还有剩余票数 | "投票成功，今天还可投X票" | 变为灰色「已投票」，票数+1 |
| 投票成功，用完最后一票 | "投票成功，今日票数已用完" | 变为灰色「已投票」，票数+1 |
| 重复点击已投过的作品 | "您已投过此作品" | 无变化 |
| 当日票数已用完 | "您今天的投票次数已用完" | 无变化 |
| 社区资格不符 | "本活动仅限XX居民投票" | 无变化 |
| 未绑定手机号 | "请先绑定手机号" | 跳转手机号绑定页 |
| 投票尚未开始 | "投票尚未开始" | 无变化 |
| 投票已结束 | "投票已结束" | 无变化 |
| 网络异常 | "网络异常，请稍后重试" | 无变化 |
...
排序控件为分段选择器（非 Tab 栏），位于候选作品标题右侧，选中项白底凸起、未选中灰底：
...
按编号/按票数切换时前端排序即可，社区排名需前端按所属社区筛选。
...
点击「查看」打开，弹窗宽度 600px，内容分为两个区域：

#### ① 作品展示区

- 图片区：多图横向滚动预览（点击可放大全屏查看），高度 300px
- 视频区：如有视频，图片下方显示视频播放器
- 展示区背景为浅灰色
...
- 标题："确认通过该作品？"
- 图标：蓝色信息图标
- 说明文案："审核通过后，该作品将进入投票池并自动分配编号。系统将向参赛者发送审核通过通知。"
- 作品信息摘要：作品名称 + 参赛者 + 所属社区（灰底信息条）
- 按钮：「取消」+「确定通过」

### 5. 驳回弹窗

- 标题："驳回作品"
- 作品信息摘要：作品名称 + 参赛者（灰底信息条）
- 必填字段：

| 字段 | 控件类型 | 规则说明 |
|---|---|---|
| [驳回原因] | 文本域 | 必填，最多 200 字，placeholder "请输入驳回原因，将通知参赛者" |

- 按钮：「取消」+「确定驳回」（红色按钮）
- 确定后：作品状态变为[已驳回]，系统发送驳回通知（含驳回原因），参赛者可在【我的投票 - 我的参赛】查看并修改重新提交

### 6. 手动添加作品弹窗

管理员为未开放报名的活动手动添加候选作品，或补充线下征集的作品。
...
| [作品名称] | 文本输入 | 必填，最多 30 字 |
| [作品介绍] | 文本域 | 必填，最多 500 字 |
| [作品图片] | 图片上传 | 必填，最少 1 张，最多 9 张 |
| [作品视频] | 视频上传 | 选填，最多 1 个 |
| [联系人姓名] | 文本输入 | 必填，作为奖励发放和沟通的联系人 |
| [联系电话] | 文本输入 | 必填 |
| [参赛分组] | 下拉选择 | 活动配置了分组时显示 |

- 手动添加的作品默认状态为[已通过]，自动分配编号，无需审核流程
- [报名者用户ID]为空（非用户自主报名），奖励发放时通过联系人信息联系
- 按钮：「取消」+「确定添加」
...
点击「投票明细」打开侧滑抽屉弹窗（宽度 720px），顶部显示关联的作品名称和当前票数。
...
| [投票IP] | IP 地址 |
| [状态] | 标签样式：有效(绿) / 已冻结(红) |
| [操作] | 有效状态显示「冻结」；已冻结状态显示「恢复」 |

- **行内操作**：
  - 「冻结」：二次确认"确认冻结该条投票？冻结后该票不计入有效票数。"，确认后状态变为[已冻结]，作品有效票数 -1，排名实时重算
  - 「恢复」：二次确认"确认恢复该条投票？恢复后该票重新计入有效票数。"，确认后状态变为[有效]，作品有效票数 +1
- **分页**：默认每页 50 条
- **底部汇总**：有效票数 X 条 / 已冻结 X 条 / 总计 X 条

### 5. 批量冻结弹窗

从排名列表点击「冻结票数」打开弹窗，标题："批量冻结 - {作品名称}"

| 字段 | 控件类型 | 规则说明 |
|---|---|---|
| [冻结方式] | 单选组 | 按 IP 冻结 / 按时间段冻结 |
| [IP 地址] | 文本输入 | 冻结方式为"按 IP"时显示，输入目标 IP |
| [时间范围] | 日期时间范围 | 冻结方式为"按时间段"时显示 |
| [预览影响] | 只读展示 | 选择条件后实时显示"将冻结 N 条投票记录，有效票数将减少 N" |

- 按钮：「取消」+「确定冻结」（红色按钮）
- 冻结后：
  - 匹配条件的投票记录批量变为[已冻结]
  - 作品有效票数减少，排名自动重算
  - 冻结操作写入审计日志（操作人、操作时间、冻结条件、影响条数）
...
|---|---|
| [作品名称] | 关联的作品名称 |
| [参赛者] | 报名者姓名 / 联系人姓名（手动添加的作品） |
| [排名] | 总排名第 N 名 |
| [奖励积分] | X 积分 |
| [积分支出方] | 活动举办社区账户 / 平台账户 |

- 按钮：「取消」+「确定发放」
- 发放成功后弹出 Toast："发放成功，X 积分已到账"
...
| [参赛者] | 姓名 |
| [奖励积分] | 该名次对应的积分值 |
| [发放状态] | 待发放 / 已发放（已发放的自动跳过） |

- 底部汇总：本次将发放 N 人，共计 X 积分，积分支出方为 XX
- 按钮：「取消」+「确定批量发放」
- 发放过程中显示进度条，全部完成后 Toast："批量发放完成，共 N 人 X 积分"
- 已发放过的作品在清单中标记"已发放"并置灰，不重复发放
...
- 发放成功后：
  - [发放状态]变为"已发放"
  - 系统发送订阅消息通知参赛者（见【订阅消息 - 2. 提醒规则 - ③ 活动奖励到账通知】）
  - 积分流水备注为"投票活动奖励-{活动名称}"
  - 积分支出方为活动举办方账户
- 已发放的奖励不可撤销
...
**触发事件**：管理员在后台对用户提交的参赛作品执行审核操作（通过或驳回）时触发。
...
**落地页**：审核通过跳转作品详情页；审核驳回跳转【我的投票 - 我的参赛】，用户可修改后重新提交。

**频次控制**：无特殊限制，每次审核操作即时发送一条。

#### ② 活动状态变更通知

**触发事件**：活动状态从[报名中]流转为[投票中]时触发，通知活动投票范围内的所有用户。
...
|---|---|
| 社区通知 | 投票活动通知 |
| 通知内容 | 「{举办社区} - {活动名称}」投票已开始，快来为喜欢的作品投票吧 |
| 通知时间 | 投票开始时间，格式 YYYY-MM-DD HH:mm |
| 提示说明 | 点击进入活动投票 |

**落地页**：点击跳转【活动详情与投票页】。

**频次控制**：同一活动的状态变更通知，每个用户只收到一次。

#### ③ 活动奖励到账通知

**触发事件**：管理员在后台对参赛作品执行奖励发放操作后触发。
...
**频次控制**：无特殊限制，每次发放操作即时发送一条。

### 3. 订阅授权触发流程

- **触发场景**：用户首次进入投票活动详情页时，若检测到总开关未授权，弹出微信订阅消息授权弹窗
- **判断流程**：检查总开关状态 → 未授权则弹窗引导 → 用户同意后开启总开关 + 投票活动子开关默认全开
- **防打扰规则**：同一用户 24 小时内最多触发 1 次授权弹窗；用户主动关闭后不再自动弹出，需在【消息订阅设置】页手动开启
...
- 超级管理员可查看和管理所有社区的活动
- 跨社区投票（如全区评选）需由超级管理员创建，活动[举办社区]设置为上级组织节点
...
| 表名 | 用途 | 关键字段 |
|---|---|---|
| 投票活动 | 活动主表，管理活动生命周期 | 活动名称、分类、状态、封面图列表（JSON，最多5张，支持多图轮播）、活动描述（富文本）、报名时间、投票时间、公示天数、投票规则（JSON）、奖励配置（JSON）、举办社区ID、举办社区名称、报名范围、投票范围、范围社区列表（JSON）、是否开放报名、是否显示实时票数、是否需要手机号 |
| 活动分组 | 活动下的分组配置 | 活动ID、分组名称、排序序号 |
| 候选作品 | 参赛作品/候选项 | 活动ID、作品名称、介绍、图片列表（JSON，最多9张）、视频地址、分组ID、报名者用户ID（可空，管理员手动添加时为空）、联系人姓名、联系电话、所属社区ID、所属社区名称、审核状态、审核时间、驳回原因、编号（审核通过后分配）、票数（冗余）、提交时间、提交方式（用户报名/管理员添加） |
| 投票记录 | 每次投票的流水（只增不改） | 活动ID、作品ID、投票用户ID、openid、投票者社区ID、投票IP、设备信息、投票日期、状态 |
| 奖励记录 | 活动结束后的奖励发放 | 活动ID、作品ID、用户ID（可空）、联系人姓名、奖励类型（总排名奖/社区内排名奖）、名次、奖励积分值、发放人ID、发放时间、积分流水ID |
| 操作审计日志 | 管理员敏感操作记录 | 活动ID、操作人ID、操作人姓名、操作类型（冻结/恢复/发放奖励/暂停/删除）、操作时间、操作详情（JSON，含条件和影响范围）、操作前值、操作后值 |
...
| [提交方式] | · 用户报名：用户通过C端报名页自主提交<br>· 管理员添加：管理员通过后台手动添加，默认已通过 |
...
| 字段 | 取值 |
|---|---|
| [操作类型] | · 冻结投票：管理员冻结可疑投票记录<br>· 恢复投票：管理员恢复被冻结的投票记录<br>· 发放奖励：管理员发放积分奖励<br>· 暂停活动：管理员暂停投票<br>· 恢复活动：管理员恢复已暂停的投票<br>· 删除活动：管理员软删除活动 |
